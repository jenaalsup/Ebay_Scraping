...
def parse_available(brand):
   ...
   # scraped_products = []
    ...
    # change spaces in the brand name to '%20' to match The Real Real url
    url_brand = (brand.replace(' ', '%20')).lower()

    while True:
        url = 'https://www.therealreal.com/products?keywords={0}&page={1}'.format(url_brand, page_num) # sorts by newly listed
        ...
        if failed:
            print("The Real Real network is unresponsive. Please try again later (or now).")
            return []

        #print("text: ", parser.text)
        product_listings = parser.xpath('//div[contains(@class, "product-card-aligner")]')
        #print(product_listings)
 
        count = 0
        for product in product_listings:
            raw_url = product.xpath('.//a[contains(@class,"product-card")]/@href')
            print("RAW URL: ", raw_url)
            raw_title = product.xpath('.//a[contains(@class,"product-card__description")]//text()')
